...
from basenet.hrnet_config import cfg
from basenet.hrnet_config import update_config
...
class PoseHigherResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        final_outputs.append(self.upsampling(y))
        ...
        final_outputs = [o.permute(0,2,3,1) for o in final_outputs]
...
def Higher_HRNet32(num_classes, **kwargs):
    update_config(cfg, "./basenet/hrnet_config/higher_hrnet_w32.yaml")
    cfg.MODEL.NUM_JOINTS = num_classes
    
    model = PoseHigherResolutionNet(cfg, **kwargs)
    
    from nsml import NSML_NFS_OUTPUT
    pretrained = os.path.join(NSML_NFS_OUTPUT, "beomyoung", cfg.MODEL.PRETRAINED)
    model.init_weights(pretrained, verbose=cfg.VERBOSE)
    
    return model


def Higher_HRNet48(num_classes, **kwargs):
    update_config(cfg, "./basenet/hrnet_config/higher_hrnet_w48.yaml")
    cfg.MODEL.NUM_JOINTS = num_classes
...
if __name__ == '__main__':
    ...
    model = Higher_HRNet32(18)
    
    outputs = model(torch.randn(2, 3, 512, 512))
